...
`today` 配列の各ユーザーには `user_id`, `username`, `name`, `followers`, `source_tweets`, `likes_count`, `source` フィールドがある。CiC操作では `username`（navigate先）と `likes_count`（いいね数）を主に使用する。
...
collect_likers.py 実行から CiC 開始までの間に別セッション（手動実行や定期タスク）が同じユーザーを先に処理する可能性がある。その場合に二重いいねを防ぐセーフティネット。
...
- **リポスト検出方法**: ツイート上部に「〇〇さんがリポスト」のラベルがある → そのツイートはスキップ対象。findの戻り値だけでは判別不可なので、navigate後に `read_page` または `get_page_text` でページテキストを取得し、「〇〇さんがリポスト」ラベルの有無を確認する
...
- 予算上限到達時は一部ユーザーが丸ごと翌日持ち越し（remaining）になる。CiC実行中に予算に達した場合は途中で打ち切り、実際にいいねした数を `liked_count` に記録する
