...
- [ ] **サービス名略語の使用**（「HN」→「Hacker News」、「PH」→「Product Hunt」等、正式名称で表記）

### 3. 改行チェック（CRITICAL強化）
- [ ] **1つの文・1つの文章塊の途中改行が完全に禁止されているか** → 文章塊内の改行は絶対NG
- [ ] 段落間の改行は意味的区切り時のみか（2段落でも3段落でも内容次第でOK）
- [ ] 読点・接続詞で自然に文章が繋がれているか（文章塊内で改行で安易に区切っていないか）
- [ ] **短文改行多用で「カッコつけてる」感が出ていないか** → 余白演出・インフルエンサー風・ポエム風・意識高い系NG
- [ ] **空行位置がつながり優先で決まっているか** → 行数で機械的にパターン化していないか（format-rules.md参照）
- [ ] **複数案レビュー時: 全案が同じ改行パターンになっていないか** → パターン固定化はAI臭さの原因（format-rules.md line 82）

### 3.5. 表現スタイルチェック（expression-rules.md準拠）
- [ ] **自分フィルターが適用されているか** → 裸の断定・自明化表現がないか
- [ ] **柔らかさ > 情報精度の優先順位が守られているか** → 「技術ブログの一段落」に見えないか。「友達とメシ食いながら」に見えるか
- [ ] **技術トピックの固有名詞が含まれているか** → 一般概念（「並列」「エージェント」等）だけで特定不能になっていないか
- [ ] **発話リズムが不均一か** → 全体が均一に圧縮or均一に丁寧になっていないか。論理部分は非圧縮、感情部分は圧縮の混在があるか
- [ ] **事実観察で文が終わっていないか** → 内的独白の漏出（「〜しないと」「〜ちゃって」等）があるか
...
### 9. 投稿価値チェック（`common/value-rules.md` 準拠）
