...
- `interests.json`: 深度調整に使う（トピック選定フィルターではない）。high_interestには熱量、low_interestはさらっと、no_interestは熱量を入れない。ただし自分の言葉で語れる切り口があればどのカテゴリでも扱える
...
#### 5-D. ニュース系トピックの「第一報 vs 二次情報」分岐（MANDATORY）

候補がニュース系（ai_news / 規制・法律 / リリース速報等）の場合、以下の分岐を明示的に判定する:

| 分岐 | 条件 | 基準 |
|------|------|------|
| **第一報ルート** | 初報から48h以内 + まだX上で消化されていない | 速報性が命。鮮度が落ちたら即撤退 |
| **二次情報ルート** | 初報から48h以上 or 既にX上で広く議論済み | 「自分が出せる具体的な価値は何か」を事前に明文化してから進む |

**二次情報ルートの事前チェック（進む前に必ず回答）:**
1. このトピックで自分だけが出せる具体的な技術情報・開発者体験・独自分析は何か？
2. 既出ツイートと差別化できる二次情報があるか？
3. 回答が曖昧な場合 → このトピックは採用しない（一般論着地のリスク大）

#### 5-E. Grok X飽和度チェック（ニュース系トピック選定時 MANDATORY）

ニュース系トピックを候補に残す場合、X上の飽和度をGrokで事前チェックする。

**実行方法（CiC優先）:**
1. **CiC（grok.com）がアクティブな場合（優先）**: CiCでgrok.comにアクセスし、「X上で "[トピック名]" に関するツイートの飽和状況を教えて。直近48時間の言及頻度、主要な論点、まだ新鮮な切り口が残っているかを判定して」と質問
2. **CiCが使用不可の場合（フォールバック）**: `python "C:\Users\Tenormusica\scripts\grok_research.py" "[トピック名] X上の反応 飽和度" --model grok-3-mini --mode none` で簡易チェック

**判定基準:**
- 飽和度が高い（多数のアカウントが既に言及）→ 第一報ルートは不可。二次情報ルートで差別化できるか再判定
- 飽和度が低い（まだ少数のみ言及）→ 第一報ルートで速報性を活かせる

**⚠️ APIフォールバックの制限**: `grok-3-mini --mode none` はGrokのモデル知識のみで回答するため、直近48時間のX言及頻度を正確に返せない可能性がある（web/xモード非対応）。CiCが使用不可でリアルタイム飽和度が取得できない場合は、`python "C:\Users\Tenormusica\x-auto\scripts\saturation_quantifier.py"` によるtwscrape実測をフォールバックとして使用する。
...
| カテゴリ | 条件 | 判定 |
|---------|------|------|
| **規制・法律ニュース** | 自分だけが出せる具体的な技術情報・開発者体験・独自分析があるか？ | YES → 採用（通常評価へ） / NO → スキップ |

→ 「開発者として具体的にどう影響を受けるか」「APIやワークフローがどう変わるか」等の二次情報がなければスキップ。
...
■ 複数案生成時の温度感バリエーション（MANDATORY）:
  複数案（3案等）を生成する際は「切り口」だけでなく「温度感（感情軸）」も変えること。
  以下のプールから毎回ランダムに3種を選択し、各案に割り当てる（同一ランで同じ軸を2案に使わない）:
  1. 淡々蓄積系: 「前から思ってた」トーン。驚きを演出しない。〜んだよね、〜ではある
  2. 実体験系: 「試してみたら〜」当事者感。自分の実際の経験ベースで語る
  3. 軽発見系: 「知らなかった、でも力まない」。〜らしい。知らなかった
  4. 思慮系: 情報を受け取って自分の中で処理した感じ。ちょっと考えた
  5. 視点発見系: 視点の新鮮さを前面に。こういう見方があるとは
  6. 抑制衝撃系: びっくりした内容を感情を抑えて言う。まじか、という感じ
  [生成時のメモ: 今回選んだ3軸を明示する（例: 軸2・軸5・軸6 を使用）]
...
■ expression-rules.md から:
  ...
  - 助詞・目的語: [他動詞の目的語が宙に浮いていないか。「何を?」テストを適用する]
  - 述語解像度: [「来る」「ある」等の低解像度述語を使っていないか]
...
**【文体モード（MANDATORY）】**
- **常体（casual）を使用する** - `tone.json` の `formality` 設定に関わらず常体で生成する
- 敬体（〜ました / 〜です / 〜ます / 〜ています / 〜いただき等）は禁止
- politeモードは廃止済み。情報共有系・広いリーチ狙いの場合でも常体を維持する
- 敬体で書いたツイートはreview-tweetでFAIL判定になる（review-tweet準拠）
