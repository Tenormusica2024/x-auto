...
- **投稿アカウント**: @sena_09_04（表向き・CiCブラウザでログイン中）
- **API連携アカウント**: @SundererD27468（user_id: `1805557649876172801`・tweepy認証トークン紐付け）
- **使い分けの理由**: X APIのOAuth認証は@SundererD27468に紐付け済み。投稿・閲覧は@sena_09_04のブラウザセッション（CiC）で実行。いいね返し等のAPI収集は@SundererD27468経由。
