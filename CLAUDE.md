...
| 種別 | 条件 | スキル | 文字数 |
|------|------|--------|--------|
| **短文ツイート** | ネタ提供済み。ニュース速報・知見共有・BIP | `skills/generate-tweet/PROMPT.md` | 300字以内 |
...
- [2026-02-18] メタ言及 vs 実践: 「think in publicネタを作って」の指示に対し、think in publicという概念について語るメタツイートを生成した。正しくは概念を実践するツイート（思考過程そのものを公開するツイート）を書くべき → `value-rules.md` に「メタ言及の禁止」として追記済み
- [2026-02-18] 口調の柔らかさ不足: 自動投稿前提では、ユーザー介在なしで公開されるため断定的・攻撃的な口調のリスクが高い。デフォルトで中性的〜女性的な柔らかい口調を採用する → `expression-rules.md` に「中性的〜女性的な物腰の柔らかさ」として追記済み
- [2026-02-18] 中性的柔らかさのキャリブレーション: 目指すのは「中性的な柔らかさ」であって「女性口調」ではない。「のよね」等の明確に女性的な語尾は禁止。柔らかい語尾（な/かな/かも/ね/よ）は全語尾の約40%が目安で、残り60%は素の語尾（っぽい/してる/だけど等）。40%を超えて大半が柔らかくなると不自然。実際に語尾を数えてバランスを確認すること
- [2026-02-18] CLAUDE.mdスコープ明示: 「CLAUDE.mdに書く」だけではグローバルかプロジェクト単位か不明。読者が混乱する。常に「グローバルのCLAUDE.md」「プロジェクトのCLAUDE.md」と書き分ける → `value-rules.md` に「技術トピックのファイル設計ガイダンス」として追記済み
- [2026-02-18] デフォルト文字数140→300: 140字は短すぎて高品質な表現が困難（誤魔化しがきかない）。300字をデフォルトにすることで表現の幅を確保 → `format-rules.md` の短文/BIPを300字に変更済み
- [2026-02-18] バズツイートシード戦略: 「メタ言及するな、実践を見せろ」のルールだけでは自分の実体験がないトピックで品質が停滞する。蓄積バズツイートを種（シード）として活用し、方向性だけ借りて表現・構造・具体例・洞察は独自構築する。同一認識不可能レベルまで加工が必須 → `value-rules.md` に「バズツイートシード戦略」、`PROMPT.md` にステップ4.5として追記済み
- [2026-02-18] 数値の主語省略禁止: 「APIコスト$2.33→$0.27」だけでは何のAPIか読者に伝わらない。数値を出すときはサービス名・課金形態等の文脈を必ず添える。「省略による曖昧さの防止」ルールはベンチマーク以外（コスト・パフォーマンス指標等）にも適用される → `expression-rules.md` に具体例を追加済み
- [2026-02-18] 日本語文中の半角スペース禁止: AIが文節区切りとして半角スペースを入れる癖がある（例: `残す形にしたら セッション`）。人間は日本語テキスト間にスペースを入れない。英数字と日本語の間も不要 → `format-rules.md` の絶対禁止セクションに追記済み
